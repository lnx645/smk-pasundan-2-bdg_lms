...
import SiswaSecurityController from '@/actions/App/Http/Controllers/SiswaSecurityController'; // Pastikan path ini benar
import { Head, Link } from '@inertiajs/vue3';
import axios from 'axios';
import { reactive, ref } from 'vue';
import Logo from '../../../img/logo.png'; // Sesuaikan path logo
...
const showPassword = ref(false); // Untuk toggle hide/show password

// Form Data
const form = reactive({
    nis: '', // Ubah dari nisn ke nis
    answer: '',
    new_password: '',
});
...
const checkNis = async () => {
    ...
    try {
        // Menggunakan URL dari controller action helper kamu
        const url = SiswaSecurityController.checkNisn().url;
        // Payload dikirim sebagai { nis: ... } sesuai validasi controller
        const res = await axios.post(url, { nis: form.nis });

        securityQuestion.value = res.data.question;
        step.value = 2; // Pindah ke step jawab pertanyaan
    } catch (error) {
...
const doReset = async () => {
    ...
    try {
        ...
        // Kita kirim 'nis' (bukan nisn) agar konsisten, pastikan controller menerima 'nis' juga
...
<template>
    ...
    <div class="flex min-h-screen flex-col items-center justify-start bg-white pt-10 font-sans">
        ...
        <div class="w-full max-w-sm rounded-[20px] bg-[#F5F5F5] p-8 shadow-sm">
            <div class="mb-6">
                <h3 class="inline-block w-full border-b-2 border-gray-400 pb-2 text-lg font-bold text-black">Reset Password</h3>
            ...
            <div v-if="step === 1">
                <div class="mb-6">
                    <label class="mb-2 block pl-1 text-sm text-black"> NIS </label>
                    <input
                        v-model="form.nis"
                        type="text"
                ...
                <button
                    @click="checkNis"
                    :disabled="loading"
                    class="w-full rounded-full bg-[#0D1B6E] py-3 text-sm font-bold tracking-wider text-white uppercase shadow-md transition-colors hover:bg-blue-900 disabled:opacity-70"
                >
            ...
            <div v-else-if="step === 2">
                <div class="mb-6 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
                    <p class="mb-1 text-xs font-bold text-[#C85200] uppercase">Pertanyaan Keamanan:</p>
                    <p class="text-sm font-medium text-gray-800">"{{ securityQuestion }}"</p>
                </div>

                <div class="mb-4">
                    <label class="mb-2 block pl-1 text-sm text-black"> Jawaban Anda </label>
                    <input
                        v-model="form.answer"
                        type="text"
                ...
                <div class="mb-8">
                    <label class="mb-2 block pl-1 text-sm text-black"> Password Baru </label>
                    <div class="relative">
                        <input
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <button @click="step = 1" class="text-center text-xs text-gray-500 underline hover:text-[#0D1B6E]">Kembali ke Cek NIS</button>
                </div>
            </div>

            <div v-if="errorMessage" class="mt-4 rounded bg-red-50 p-2 text-center text-xs font-semibold text-red-500">
                {{ errorMessage }}
            </div>

            <div class="mt-6 flex justify-center border-t border-gray-300 pt-4">
                <Link :href="LoginController().url" class="text-xs font-bold text-gray-500 transition-colors hover:text-[#0D1B6E]">
