...
<template>
    ...
    <div class="mx-auto mt-6 flex max-w-full flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-neutral-100 bg-neutral-50/50 px-4 py-3">
            <div class="relative w-48">
                <select
                    ...
                    class="w-full cursor-pointer appearance-none rounded-md border border-neutral-200 bg-white py-1.5 pr-8 pl-3 text-xs font-medium text-neutral-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
                >
        ...
        <div v-if="($page.props.matpel_kode || $page.props?.materials) && $page.props?.materials.length > 0">
            <ul class="divide-y divide-neutral-100">
                <li
                    ...
                    :key="materi.id"
                    class="group flex items-center justify-between gap-3 px-4 py-3 transition-colors hover:bg-blue-50/40"
                >
                    <div class="flex min-w-0 flex-1 items-center gap-3">
                        <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-xs font-bold text-blue-600">
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="mb-0.5 flex items-center gap-2">
                                <h4 class="truncate text-sm font-semibold text-neutral-800">
                                    {{ materi.title }} <template v-if="materi.kategori_materi">- {{ materi.kategori_materi }}</template>
                                </h4>
                                <span
                                    v-if="materi.jumlahFileMateri > 0"
                                    class="inline-flex items-center gap-0.5 rounded border border-neutral-200 bg-neutral-100 px-1.5 py-0.5 text-[10px] font-medium text-neutral-500"
                                >
                            ...
                            <div class="flex items-center gap-3 text-xs text-neutral-400">
                                ...
                                <span :class="materi.is_published ? 'font-medium text-green-600' : 'font-medium text-amber-600'">
                    ...
                    <div class="flex items-center gap-2 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
                        <Button
                            ...
                            variant="secondary"
                            class="!h-7 border border-neutral-200 bg-white !px-2 !text-[10px] hover:border-green-200 hover:bg-green-50 hover:text-green-700"
                        >
                            Publish
                        </Button>
                        ...
                        <button
                            ...
                            class="flex h-7 w-7 items-center justify-center rounded text-neutral-400 transition-colors hover:bg-red-50 hover:text-red-500"
                            title="Hapus"
                        >
